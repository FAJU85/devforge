...
case $TEST_TYPE in
    all)
        ...
        pytest . -v --alluredir="$ALLURE_RESULTS" 2>&1 | tee qa-selenium-results.log
        ;;
    smoke)
        echo -e "${YELLOW}   Running smoke tests only...${NC}"
        pytest . -v -m smoke --alluredir="$ALLURE_RESULTS" 2>&1 | tee qa-selenium-results.log
        ;;
    selenium)
        echo -e "${YELLOW}   Running Selenium tests only...${NC}"
        pytest selenium/ -v --alluredir="$ALLURE_RESULTS" 2>&1 | tee qa-selenium-results.log
        ;;
    *)
